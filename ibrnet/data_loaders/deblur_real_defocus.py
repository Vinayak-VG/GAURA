...
class DeblurRealDefocus(Dataset):
    
    def __init__(self, args, mode, scenes=(), random_crop=True, **kwargs):
        self.folder_path = os.path.join("/data/cilab4090/Universal_NeRF_Baselines/real_data/blur/real_defocus_blur")
        self.args = args
        mode = "test"
        ...
        for i, scene in enumerate(scenes):
            ...
            if scene == "defocusbush":
                i_test = np.arange(poses.shape[0])[:: 13]
            elif scene == "defocuscake":
                i_test = np.arange(poses.shape[0])[:: 9]
            elif scene == "defocuscaps":
                i_test = np.arange(poses.shape[0])[:: 5]
            elif scene == "defocuscisco":
                i_test = np.arange(poses.shape[0])[:: 7]
            elif scene == "defocuscoral":
                i_test = np.arange(poses.shape[0])[:: 10]
            elif scene == "defocuscupcake":
                i_test = np.arange(poses.shape[0])[:: 6]
            elif scene == "defocuscups":
                i_test = np.arange(poses.shape[0])[:: 8]
            elif scene == "defocusdaisy":
                i_test = np.arange(poses.shape[0])[:: 8]
            elif scene == "defocussausage":
                i_test = np.arange(poses.shape[0])[:: 6]
            elif scene == "defocusseal":
                i_test = np.arange(poses.shape[0])[:: 6]
            elif scene == "defocustools":
                i_test = np.arange(poses.shape[0])[:: 7]
            i_train = np.array(
                [
                    j
