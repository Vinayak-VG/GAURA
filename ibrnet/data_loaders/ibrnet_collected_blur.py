...
import random
from ibrnet.data_loaders.create_blur import gen_blur

class IBRNetCollectedBlurDataset(Dataset):
    
    def __init__(self, args, mode, random_crop=True, **kwargs):
        
        # if args.ft_corrup == "motion":
        #     self.CORRUPTIONS_in = [
        #     ("_motionblur1", 0),
        #     ]
        #     # self.CORRUPTIONS_out = [
        #     # ("", 1),
        # # ]
        
        self.args = args
        self.folder_path1 = os.path.join("/data/New_Data_Gen/data/ibrnet_collected_1/")
        self.folder_path2 = os.path.join("/data/New_Data_Gen/data/ibrnet_collected_2/")
    ...
    def __getitem__(self, idx):
        # postfix1, embed_id1 = random.choice(self.CORRUPTIONS_in)
        # postfix2, embed_id2 = random.choice(self.CORRUPTIONS_out)
        ...
        for id in nearest_pose_ids:
            ...
            if self.args.ft_corrup == "motion":
                size = np.random.randint(10, 45)
                angle_list = list(range(0, 15)) + list(range(75, 105)) + list(range(165, 180))
                angle = random.choice(angle_list)
                src_rgb = (gen_blur.gen_blur(src_rgb_file, size, angle, self.args.ft_corrup) / 255.0).astype(np.float32)
            elif self.args.ft_corrup == "defocus":
                blur_amount = np.random.randint(5, 20)
                depth_list = [-1, 1]
                depth_type = random.choice(depth_list)
                src_rgb = (gen_blur.defocus_gen_blur(src_rgb_file, src_rgb_file_depth, blur_amount, depth_type) / 255.0).astype(np.float32)
        ...
        return {
            ...
            "embed_id1": torch.LongTensor([0]),
            "embed_id2": torch.LongTensor([0]),
        }
