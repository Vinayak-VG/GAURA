...
class LLFFRenderDataset(Dataset):
    def __init__(self, args,
        ...
        for i, scene in enumerate(scenes):
            ...
            if "still" in self.args.eval_scenes[0]:
                _, poses, bds, render_poses, i_test, rgb_files = load_llff_data(scene_path, load_imgs=False, factor=1)
            elif "haze" in self.args.eval_scenes[0]:
                if "W002" not in self.args.eval_scenes[0]:
                    _, poses, bds, render_poses, i_test, rgb_files = load_llff_data(scene_path, load_imgs=False, factor=2)
                else:
                    _, poses, bds, render_poses, i_test, rgb_files = load_llff_data(scene_path, load_imgs=False, factor=4)
            elif "rain" in self.args.eval_scenes[0]:
                if "rb3" in self.args.eval_scenes[0]:
                    _, poses, bds, render_poses, i_test, rgb_files = load_llff_data(scene_path, load_imgs=False, factor=1)
                else:
                    _, poses, bds, render_poses, i_test, rgb_files = load_llff_data(scene_path, load_imgs=False, factor=2)
            elif "snow" in self.args.eval_scenes[0]:
                _, poses, bds, render_poses, i_test, rgb_files = load_llff_data(scene_path, load_imgs=False, factor=2)
            elif "blur" in self.args.eval_scenes[0]:
                _, poses, bds, render_poses, i_test, rgb_files = load_llff_data(scene_path, load_imgs=False, factor=4)
            elif "defocus" in self.args.eval_scenes[0]:
                _, poses, bds, render_poses, i_test, rgb_files = load_llff_data(scene_path, load_imgs=False, factor=4)
    ...
    def __getitem__(self, idx):
        ...
        for id in nearest_pose_ids:
            ...
            src_rgb_file = os.path.join(os.path.dirname(src_rgb_file) + postfix, os.path.basename(src_rgb_file))
        ...
        return {'camera': torch.from_numpy(camera),
                ...
                "embed_id1": torch.LongTensor([embed_id]),
                }