...
class NerfSyntheticRenderDataset(Dataset):
    def __init__(
        ...
        **kwargs
        ...
        self.folder_path = os.path.join("/home/vinayak/restoration_nerf/dark/LOM_full")
        ...
        for scene in scenes:
            ...
            for i in range(len(rgb_files)):
                # print(rgb_files[i])
                rgb_files[i] = rgb_files[i].replace("gh", "high")
            # if self.mode != "train":
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "train":
            rgb_file_test = rgb_file.replace("high", "low")
            id_render = int(os.path.basename(rgb_file_test)[:-4].split("_")[1])
        ...
        for id in nearest_pose_ids:
            ...
            src_rgb_file = os.path.join(os.path.dirname(src_rgb_file) + "_dark1", os.path.basename(src_rgb_file))
            src_rgb = imageio.imread(src_rgb_file.replace("/w", "/low")).astype(np.float32) / 255.0
