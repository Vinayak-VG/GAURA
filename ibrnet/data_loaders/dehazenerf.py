...
class DehazeNeRFReal(Dataset):
    
    def __init__(self, args, mode, scenes=(), random_crop=True, **kwargs):
        self.folder_path = os.path.join("/data/restoration_nerf/haze/dehazenerf/real_haze/")
        self.args = args
        mode = "test"
        ...
        for i, scene in enumerate(scenes):
            ...
            if scene == "bear":
                i_test = np.arange(poses.shape[0])[:47]
            elif scene == "elephant":
                i_test = np.arange(poses.shape[0])[58:]
            elif scene == "lion":
                i_test = np.arange(poses.shape[0])[:47]
            i_train = np.array(
                [
