    ...
    def __init__(self, args, mode, scenes=(), random_crop=True, **kwargs):
        ...
        for i, scene in enumerate(scenes):
            ...
            _, poses, bds, render_poses, intrinsic, i_test = load_llff_data(
                scene_path, load_imgs=False, factor=4, render_style='', split_train_val=0
            )
            # print(i_test)
            # exit()
            if len(intrinsic) == 3:
            ...
            # intrinsics = np.eye(4)
            # # intrinsic = np.array([[fx, 0, -cx, 0], [0, -fy, -cy, 0], [0, 0, 1, 0]]).astype(np.float32)
            # # intrinsic = intrinsic / 1 
            # intrinsics[:3, :4] = intrinsic
            ...
            # print(c2w_mats.shape, intrinsics.shape)
            # exit()
