...
def read_cameras(pose_file, test=False):
    ...
    rgb_file = os.path.join(basedir, meta["frames"][0]["file_path"][2:])
    if test:
        rgb_file = rgb_file.replace("nerf-dataset", "nerf-cmap").replace("test/", "").replace("images/", "")
    ...
    for i, frame in enumerate(meta["frames"]):
        rgb_file = os.path.join(basedir, meta["frames"][i]["file_path"][2:])
        if test:
            rgb_file = rgb_file.replace("nerf-dataset", "nerf-cmap").replace("test/", "").replace("images/", "")
...
class RavenDataset(Dataset):
    def __init__(
        ...
        **kwargs
        ...
        self.folder_path = os.path.join(args.rootdir, "data/block-insertion-sixdof-train/nerf-dataset")
        ...
        for i, scene in enumerate(scenes):
            ...
            self.train_rgb_files.append(rgb_files)
            self.train_poses.append(poses)
            self.train_intrinsics.append(intrinsics)

            pose_file = os.path.join(self.scene_path, "test", "transforms_test.json")
            render_rgb_files, render_intrinsics, render_poses = read_cameras(pose_file, test=True)

            if self.mode != "train":
                render_rgb_files = render_rgb_files[:: self.testskip]
                render_intrinsics = render_intrinsics[:: self.testskip]
                render_poses = render_poses[:: self.testskip]
