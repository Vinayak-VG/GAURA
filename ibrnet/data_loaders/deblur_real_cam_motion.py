...
class DeblurRealMotion(Dataset):
    
    def __init__(self, args, mode, scenes=(), random_crop=True, **kwargs):
        self.folder_path = os.path.join("/home/vinayak/restoration_nerf_finalsprint/real_data/blur/real_camera_motion_blur")
        self.args = args
        mode = "test"
        ...
        for i, scene in enumerate(scenes):
            ...
            elif scene == "blurbasket":
                i_test = np.arange(poses.shape[0])[:: 7]
            elif scene == "blurbuick":
                i_test = np.arange(poses.shape[0])[:: 7]
            elif scene == "blurcoffee":
                i_test = np.arange(poses.shape[0])[:: 6]
            elif scene == "blurdecoration":
                i_test = np.arange(poses.shape[0])[:: 6]
            elif scene == "blurgirl":
                i_test = np.arange(poses.shape[0])[:: 7]
            elif scene == "blurheron":
                i_test = np.arange(poses.shape[0])[:: 8]
            elif scene == "blurparterre":
                i_test = np.arange(poses.shape[0])[:: 6]
            elif scene == "blurpuppet":
                i_test = np.arange(poses.shape[0])[:: 6]
            elif scene == "blurstair":
                i_test = np.arange(poses.shape[0])[:: 6]
            i_train = np.array(
                [
                    j
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "embed_id1": torch.LongTensor([3]),
            # "embed_id2": torch.LongTensor([3]),
        }