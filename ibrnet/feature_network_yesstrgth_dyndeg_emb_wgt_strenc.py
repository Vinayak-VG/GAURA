...
import importlib
from ibrnet.prompt_block_yesstrgth_dyndeg_emb_wgt_strenc import PromptBlock
from ibrnet.regressor import EstimationRegressorModel
...
class ResUNet(nn.Module):
    def __init__(
        ...
        num_ids = 7
        self.promptblock1 = PromptBlock(prompt_dim = 128, lin_dim = 256, num_ids = num_ids)
        self.promptblock2 = PromptBlock(prompt_dim = 64, lin_dim = 128, num_ids = num_ids)
        self.promptblock3 = PromptBlock(prompt_dim = 16, lin_dim = 32, num_ids = num_ids)
        self.strength_encoders = nn.ModuleList([])
        for i in range(num_ids):
            self.strength_encoders.append(EstimationRegressorModel(BasicBlock, [1, 1, 1, 1]))
        # self.auxillary_rgb = nn.Sequential(
    ...
    def forward(self, x, embed_id):
        # print(x.shape)
        # quit()
        strength, regressed = self.strength_encoders[embed_id.item()](x[0].unsqueeze(0))
        x = self.relu(self.bn1(self.conv1(x)))

        x1 = self.layer1(x)
        x2 = self.layer2(x1)
        x3 = self.layer3(x2)
        x3 = self.promptblock1(x3, embed_id, strength)

        x = self.upconv3(x3)
        x = self.skipconnect(x2, x)
        x = self.iconv3(x)
        
        x = self.promptblock2(x, embed_id, strength)
        
        x = self.upconv2(x)
        x = self.skipconnect(x1, x)
        x = self.iconv2(x)
        
        x = self.promptblock3(x, embed_id, strength)
        
        x_out = self.out_conv(x)
        
        # aux_rgb = self.auxillary_rgb(x_out)
        if self.coarse_only:
            x_coarse = x_out
            x_fine = None
        else:
        ...
        # e_out = [e_out1, e_out2, e_out3]
        return x_coarse, x_fine, strength, regressed



if __name__ == "__main__":
    x = torch.randn((2, 3, 578, 304)).to("cuda")
    # model = GradualStyleBlock(3, 64).to("cuda")
    model = EstimationRegressorModel(BasicBlock, [1, 1, 1, 1]).to("cuda")
    x, y = model(x)
