...
class Attention2D(nn.Module):
    def __init__(self, dim, dp_rate, num_ids):
        ...
        self.dp = nn.Dropout(dp_rate)
        self.strength_embedding = _get_sinusoid_encoding_table(3, dim)

    def forward(self, q, k, pos, mask=None, embed_id1=None, embed_id2=None, strength_emb1=None):
        if embed_id1.item() != 0:
            strength_id = (embed_id1.item() - 1) % 3
            strength_emb1 = self.strength_embedding[strength_id]
            k_embed = self.k_embeds(torch.LongTensor([(embed_id1.item()-1)//3 + 1]).to("cuda"))
            v_embed = self.v_embeds(torch.LongTensor([(embed_id1.item()-1)//3 + 1]).to("cuda"))
        else:
            k_embed = self.k_embeds(torch.LongTensor([0]).to("cuda"))
            v_embed = self.v_embeds(torch.LongTensor([0]).to("cuda"))
            strength_emb1 = torch.zeros((self.dim)).to("cuda")

        if embed_id2.item() != 0:
            strength_id = (embed_id2.item() - 1) % 3
            strength_emb2 = self.strength_embedding[strength_id]
            q_embed = self.q_embeds(torch.LongTensor([(embed_id2.item()-1)//3 + 1]).to("cuda"))
        else:
            q_embed = self.q_embeds(torch.LongTensor([0]).to("cuda"))
            strength_emb2 = torch.zeros((self.dim)).to("cuda")
        ...
        q = q + strength_emb2.unsqueeze(0).unsqueeze(0)

        # k = F.linear(k, k_embed)
        # k = k * F.sigmoid(self.transform_embed(strength_emb1))
        k = F.linear(k, k_embed)
        k = k + strength_emb1.unsqueeze(0).unsqueeze(0)

        v = F.linear(k, v_embed)
        v = v + strength_emb1.unsqueeze(0).unsqueeze(0)
...
class Transformer2D(nn.Module):
    ...
    def forward(self, q, k, pos, mask=None, embed_id1=None, embed_id2=None, strength_emb1=None):
        residue = q
        x = self.attn_norm(q)
        x = self.attn(x, k, pos, mask, embed_id1=embed_id1, embed_id2=embed_id2, strength_emb1=strength_emb1)
        x = x + residue
...
class Attention(nn.Module):
    ...
    def forward(self, x, pos=None, ret_attn=False, embed_id1=None, embed_id2=None):
        # if embed_id1.item() != 0:
        #     embed1 = self.embeds(torch.LongTensor([(embed_id1.item()-1)//3 + 1]).to("cuda"))
        # else:
        #     embed1 = self.embeds(torch.LongTensor([0]).to("cuda"))

        if embed_id2.item() != 0:
            strength_id = (embed_id2.item() - 1) % 3
            strength_emb2 = self.strength_embedding[strength_id]
            embed2 = self.embeds(torch.LongTensor([(embed_id2.item()-1)//3 + 1]).to("cuda"))
        else:
            embed2 = self.embeds(torch.LongTensor([0]).to("cuda"))
            strength_emb2 = torch.zeros((self.dim)).to("cuda")
        
        # embed = torch.cat((embed1, embed2), dim=1)
        # embed = self.embed_linear(embed)
        # embed1 = embed1.view(self.dim, self.dim)
        embed2 = embed2.view(self.dim, self.dim)
        ...
        v = F.linear(x, embed2)
        v = v + strength_emb2.unsqueeze(0).unsqueeze(0)
...
class Transformer(nn.Module):
    ...
    def forward(self, x, pos=None, ret_attn=False, embed_id1=None, embed_id2=None):
        residue = x
        x = self.attn_norm(x)
        x = self.attn(x, pos, ret_attn, embed_id1=embed_id1, embed_id2=embed_id2)
        if ret_attn:
            x, attn = x
        x = x + residue
...
class TransIBRNet(nn.Module):
    def __init__(self, args, in_feat_ch=32, posenc_dim=3, viewenc_dim=3, ret_alpha=False):
        ...
        for i in range(args.viewtrans_depth):
            view_crosstrans = Transformer2D(
                dim=args.netwidth, ff_hid_dim=int(args.netwidth * 4), ff_dp_rate=0.1, attn_dp_rate=0.1, num_ids=5,
            )
            self.view_crosstrans.append(view_crosstrans)
            view_selftrans = Transformer(
                dim=args.netwidth, ff_hid_dim=int(args.netwidth * 4), n_heads=4, ff_dp_rate=0.1, attn_dp_rate=0.1, num_ids=5,
            )
    ...
    def forward(self, rgb_feat, ray_diff, mask, pts, ray_d, embed_id1, embed_id2, strength_emb1):
        ...
        for i, (crosstrans, q_fc, selftrans) in enumerate(zip(self.view_crosstrans, self.q_fcs, self.view_selftrans)):
            q = crosstrans(q, rgb_feat, ray_diff, mask, embed_id1=embed_id1, embed_id2=embed_id2, strength_emb1=strength_emb1)
            if i % 2 == 0:
                q = torch.cat((q, input_pts, input_views), dim=-1)
                q = q_fc(q)
            q = selftrans(q, ret_attn=self.ret_alpha, embed_id1=embed_id1, embed_id2=embed_id2)
            if self.ret_alpha:
                q, attn = q
        h = self.norm(q)
