...
class DPT(BaseModel):
    ...
    def forward(self, x, use_features=False, return_features=False):
        if use_features:
            if self.scratch.stem_transpose is not None:
                x = self.scratch.stem_transpose(x)

            out = self.scratch.output_conv(x)
            return out
        else:
...
class DPTDepthModel(DPT):
    ...
    def forward(self, x, use_features=False, return_features=False):
        return super().forward(x, use_features=use_features, return_features=return_features).squeeze(dim=1)
